...
| `MatchCandidate.Source`/`.LastOpen`/`.MetadataAmbiguous` values | Plugin populates from live reader context (`"current_file"`, `"statistics"`, stats-derived last-open, ambiguity from duplicate stats rows) | **No such context exists in a headless bridge.** Bridge-specific constants proposed in §6.2 |
...
| `Source` | constant `"readest"` | **bridge-specific** — the plugin uses `"current_file"`/`"statistics"`/`"file"` depending on which live KOReader subsystem produced the candidate; none apply headlessly |
...
- Every hash in `resp.Unmatched`: `st.SetUnmatched(hash, now)`, then `st.DeleteMatch(hash)` (§2, verified coupling — defensive; normally there's nothing to delete for a hash that was never matched, but harmless and correct if it was previously matched and has now regressed).
- Any hash that was in the request but appears in **neither** list (defensive — should not happen per the documented contract, but the client normalizes nil to empty slices rather than guaranteeing coverage): treated as a batch-level failure for that hash for the purposes of watermark retreat (§5.4), logged at `warn`.
...
3. Fresh unmatched hash → `MatchCheck` called with correct candidate fields (`LastOpen` derived, `Source="readest"`, `MetadataAmbiguous=false`); match found → `SetMatch`+`ClearUnmatched`; always pushed regardless of pct value (including pct == 0).
...
8. `MatchCheck` returns a hash in `Unmatched` → `SetUnmatched`+`DeleteMatch`, no push, watermark still advances, next `RunOnce` doesn't recheck within cooldown.
...
- **E.** `MatchCandidate.Source`/`LastOpen`/`MetadataAmbiguous` bridge-specific values (§6.2): constant `"readest"`, `updated_at` as the `LastOpen` proxy, always `false` for ambiguity. **Alternative values welcome if a different `Source` string is preferred for BookOrbit-side diagnostics.**
Yes, with the shown bridge-specific values. Source = "readest", LastOpen = updated_at, MetadataAmbiguous = false is the cleanest headless bridge mapping. It keeps the data stable and makes diagnostics readable without pretending the bridge has live-reader context it does not have.
